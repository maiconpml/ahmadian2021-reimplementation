...
public:
	...
	void local_search(vector<unsigned>& seq, vector<unsigned>& starts, double& targetPenalties, unsigned k, unsigned iterMax) {
	...
	void vns() {
		...
		while (k <= 3) {
			...
			local_search(seq, starts, auxPenalties, k, 3);
	...
	void solve(string instPath) {
		...
		if (verifySchedule()) {
			cout << instPath << " " << result << " " << penalties << endl;
		}
	}
